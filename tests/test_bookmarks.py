...
class TestBookmarkListEndpoint(unittest.TestCase):
    ...
    def test_bookmarks_get_check_if_query_correct(self):
        # Using requests module, going out and checking that the status of this 
        # request is number 200, meaning all good
        response = requests.get('{0}/api/bookmarks'.format(root_url))
        self.assertEqual(response.status_code, 200)
        # Getting all bookmarks from the current user
        bookmarks = response.json()
        bookmark_ids = utils.get_bookmark_ids(self.current_user.get('id'))
        # Then checking that there's more than 1
        # Then checking that every bookmark is in fact one from the database
        # saved by our current user
    ...
    def test_bookmarks_get_check_if_data_structure_correct(self):
        ...
        # Making sure that the bookmark has all the things it should have
...
if __name__ == '__main__':
    ...
    suite.addTests([
        ...
        # GET (List) Tests:
        # this one is checking if we even have any bookmarks in the first place, going to endpoint and checking
